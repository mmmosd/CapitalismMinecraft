    ...
    public void SendMoneyAtoB(Player a, Player b, int amount) {
        ...
        if (Wlist.containsKey(A) && Wlist.containsKey(B)) {
            if (Wlist.get(A) >= amount) {
                ...
                a.sendMessage(Component.text(ChatColor.LIGHT_PURPLE + b.getName() + ChatColor.WHITE + "님 에게 " + ChatColor.GOLD + amount + "🪙" + ChatColor.WHITE + "를 보냈습니다!"));
                b.sendMessage(Component.text(ChatColor.LIGHT_PURPLE + a.getName() + ChatColor.WHITE + "님 에게 " + ChatColor.GOLD + amount + "🪙" + ChatColor.WHITE + "를 받았습니다!"));
